...
class ReporteController extends Controller
{
    ...
    public function reporte_estadistico(Request $request)
    {
        ...
        $totales = DB::select("SELECT date_format(s.fecha, 'TMMonth') as mes, sum(d.precio * d.cantidad) as total
                                    from sales s
                                             inner join details d on s.id = d.sales_id
                                    where extract(year  from s.fecha) = ?
                                    group by date_format(s.fecha, 'TMMonth'), extract(month from s.fecha)", [$anio]);
